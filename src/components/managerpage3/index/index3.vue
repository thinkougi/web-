    ...
    <el-table
      ...
      >
        <template slot-scope="scope">
          ...
          <router-link :to="{path:'/submitMiddle', query:{name:scope.row.name}}"><el-button type="text" size="small">提交中期报告</el-button></router-link>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    ...
    show () {
      this.$http.post('/api/user/queryProject3', {captain: this.COMMON.username1}, {}).then((res) => {
        console.log(res)
