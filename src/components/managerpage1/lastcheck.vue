  ...
  <div>
    <el-dialog title="项目申请管理" :visible.sync="dialogFormVisible" width="30%">
      <table id="td"   cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td class="tdname">中期检查：</td>
          <td class="tinfo">{{tmiddlecheck}}</td>
        </tr>
      </table>
      <el-form :model="from">
        <el-form-item  label="结题报告">
          <el-radio-group v-model="from.lastcheck">
        ...
        <el-form-item>
          <el-form-item>
            <el-button type="primary" @click="end">确认</el-button>
          </el-form-item>
        </el-form-item>
      </el-form>
...
<script>
  export default {
    ...
    methods: {
        ...
        var state = '';
        var lastcheck = this.from.lastcheck;
        if(lastcheck == '已提交'){
          state = '项目结题';
        }else if(lastcheck == '未提交'){
          state = '项目中期';
        }else {
          alert('参数错误');
        }
        this.$http
          .patch("api/user/checkUpdate2", {
            //传递的参数
            lastcheck:this.from.lastcheck,
            state:state,
            id: this.rowid
          }).then(response => {
          alert("该项目结题成功");
      ...
      getInfo:function () {
        this.$http.get('/api/user/stateNoUser2',{},{}).then((response)=>{
      ...
      handleEdit(row){
        ...
        this.$http
          ...
          .then(response => {
            ...
            this.tmiddlecheck = this.gridData[0].middlecheck;
            this.tresult = '';
            if(this.tresult1 == '1'){
              this.tresult += '实物、'
            }
