<template>
  <div>
    <el-dialog title="项目评估" :visible.sync="dialogFormVisible" width="30%">
      ...
      <el-form>
        <el-form-item :model="from" label="项目评估">
        ...
        <el-form-item>
          <el-form-item>
            <el-button type="primary" @click="end">评估</el-button>
          </el-form-item>
        </el-form-item>
      </el-form>
    ...
    <el-table
      ...
      <el-table-column label="操作" fixed="right">
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="handleEdit(scope.row)">评估</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  export default {
    ...
    methods: {
        ...
        this.$http
          .patch("api/user/assessUser", {
            //传递的参数
            assess:this.from.resource,
            id: this.rowid
          }).then(response => {
          alert("该项目评估成功");
        })
      ...
      getInfo:function () {
        //alert('111')
        this.$http.get('/api/user/stateYesUser',{},{}).then((response)=>{
